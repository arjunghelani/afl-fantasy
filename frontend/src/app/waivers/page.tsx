...
export default function WaiversPage() {
  ...
  useEffect(() => {
    async function fetchWaivers() {
      ...
      try {
        const response = await fetch(`/api/waivers/${year}`, { cache: 'no-store' });
        if (!response.ok) {
  ...
  const handleYearChange = async (playerKey: string, newYear: number) => {
    ...
    try {
      const response = await fetch(`/api/players/${encodeURIComponent(existing.playerName)}/weekly-stats?year=${newYear}`);
      if (!response.ok) {
        throw new Error('Failed to fetch player stats');
      }
